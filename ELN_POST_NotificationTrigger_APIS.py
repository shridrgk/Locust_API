...
class ELN(HttpUser):
    ...
    @task
    def NotificationTrigger(self):
        taskBodyData = open('notification.json')
        taskBody = json.load(taskBodyData)

        with self.client.post('/eln-core-svc-nfr/v1/Notification/trigger?UserId=cb222c6192cf48148f06e8e66a72d674', name='TriggerNotification', catch_response=True,
