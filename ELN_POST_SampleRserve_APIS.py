from locust import HttpUser, task, between
import json


class ELN(HttpUser):
    configFile = open('config.json')
    conf = json.load(configFile)
    i = 0
    host = "https://cloud.msa2.apps.yokogawa.build"
    headers = {
        "content-type": "application/json",
        'Authorization': 'Bearer eyJhbGciOiJIUzUxMiIsInR5cCI6IkpXVCJ9..NPtI5xgYp5HHiRntCv8MhjRTfU2GZUPPmiu3kGhXzkRCH6mJ15pwZtmdo-LgRKvpqKVn_cI2ufTX63psHIvvtg'
    }

    def counterNum(self):
        while (True):
            self.i = self.i + 1
            return self.i

    @task
    def reserveSample(self):
        taskBodyData = open('sampleReserve.json')
        taskBody = json.load(taskBodyData)
        taskBody['comment'] = 'Updated' + str(self.counterNum())

        with self.client.post('/eln-inventory-mgmt-svc-nfr/v1/Sample/01de9888-9a59-4138-9515-0def5181d106/reserve', name='SampleRserve', catch_response=True,
                              headers=self.headers, data=json.dumps(taskBody)) as response:
            if response.status_code == 200:
                response.success()
            else:
                print(response.json())
                response.failure(response)
