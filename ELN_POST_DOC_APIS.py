...
class ELN(HttpUser):
    ...
    @task
    def createDOCTask(self):
        taskBodyData = open('taskDOCBody.json')
        taskBody = json.load(taskBodyData)
        taskBody['title'] = 'DOCtaskB' + str(self.counterNum())
        taskBody['blocks'][0]['fields'][0]['value'] = 'DOCtaskB' + \
            str(self.counterNum())

        with self.client.post('/eln-project-mgmt-svc-nfr/v1/Task', name='createDOCTask', catch_response=True,
